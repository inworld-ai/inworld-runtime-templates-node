...
async function run() {
  ...
  const goalAdvancementRequest: GoalAdvancementRequest = {
    ...
    event: {
      ...
      intents: [] as IntentMatch[],
    },
    goalsForEvaluationOverride: [] as GoalForEvaluation[],
  };
 ...
 */
function getEventHistoryForScenario(scenario: string): InteractionEvent[] {
  switch (scenario) {
    case 'customer_support':
      return [
        {
 ...
 *
 * @returns {Object} Belief state - empty object as initial state, updated by the system
 * @param _scenario
 */
function getBeliefStateForScenario(_scenario: string): BeliefState {
  // BeliefState uses a `values` map structure internally.
  // Return empty object as initial state - the goal advancement system
  // will populate and return updated belief state as goals progress.
  return {};
}
 ...
 */
function getAgentInfoForScenario(scenario: string): AgentInfo {
  switch (scenario) {
    case 'customer_support':
      return {
        name: 'SupportAgent',
