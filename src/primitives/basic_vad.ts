 ...
 *
 * Usage:
 *   yarn basic-vad --audioFilePath=<path-to-audio.wav> [--mode=simple|silence|threshold|device]
 *
 * Modes:
...
Usage:
    yarn basic-vad --audioFilePath=<path-to-audio.wav> [options]
...
Examples:
    yarn basic-vad --audioFilePath=audio.wav
    yarn basic-vad --audioFilePath=audio.wav --mode=silence
    yarn basic-vad --audioFilePath=audio.wav --mode=threshold
    yarn basic-vad --audioFilePath=audio.wav --mode=device
`;
...
async function runDeviceExample(
  ...
  devices.forEach((device, index) => {
    ...
    if (device.info) {
      ...
      const freeMB = (device.info.freeMemoryBytes / (1024 * 1024)).toFixed(2);
      const totalMB = (device.info.totalMemoryBytes / (1024 * 1024)).toFixed(2);
  ...
  const selectedDevice = cudaDevice || { type: 'CPU' as const, index: 0 };
