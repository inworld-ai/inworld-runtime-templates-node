 ...
 *
 * Usage:
 * yarn basic-aec-filter --mode=basic
 * yarn basic-aec-filter --mode=realtime --filterLength=200
 * yarn basic-aec-filter --mode=streaming
 * yarn basic-aec-filter --mode=batch
 */
 ...
 */
async function runBasicExample(audioFile: string, filterLength: number) {
  console.log('\n=== Basic AEC Example ===\n');
  ...
  try {
    ...
    // Create AEC filter
    console.log(`\nCreating AEC filter (filter length: ${filterLength}ms)...`);
    const aecFilter = await AECFilter.create({
      localConfig: {
        filterLengthMs: filterLength,
    ...
    const hasEcho = await aecFilter.detectEcho(
      { data: new Float32Array(micAudio), sampleRate: audio.sampleRate },
      { data: new Float32Array(speakerAudio), sampleRate: audio.sampleRate },
    );
    ...
    const filteredSNR = calculateSNR(
      audio.data,
      filtered.data || new Float32Array(0),
    );
 ...
 */
async function runRealtimeExample(audioFile: string, filterLength: number) {
  console.log('\n=== Real-time AEC Example ===\n');
  ...
  try {
    ...
    // Create AEC filter
    console.log(`Creating AEC filter (filter length: ${filterLength}ms)...\n`);
    const aecFilter = await AECFilter.create({
      localConfig: {
        filterLengthMs: filterLength,
        device: { type: 'CPU', index: 0 },
    ...
    for (let i = 0; i < audio.data.length; i += chunkSize) {
      ...
      // Filter the chunk
      const filtered = await aecFilter.filterAudio(
        { data: new Float32Array(micChunk), sampleRate: audio.sampleRate },
        { data: new Float32Array(speakerChunk), sampleRate: audio.sampleRate },
      );

      totalProcessed += chunk.length;
      totalFiltered += filtered.data?.length || 0;
      ...
      if (i % (chunkSize * 5) === 0) {
        ...
        const hasEcho = await aecFilter.detectEcho(
          { data: new Float32Array(micChunk), sampleRate: audio.sampleRate },
          {
            data: new Float32Array(speakerChunk),
 ...
 */
async function runStreamingExample(audioFile: string, filterLength: number) {
  console.log('\n=== Streaming AEC Example ===\n');
  ...
  try {
    ...
    async function* createMicStream(): AsyncGenerator<{
      ...
      for (let i = 0; i < audio.data.length; i += chunkSize) {
        ...
        yield {
          data: new Float32Array(micChunk),
          sampleRate: audio.sampleRate,
        };
 ...
 */
async function runBatchExample(audioFile: string, filterLength: number) {
  console.log('\n=== Batch AEC Example ===\n');
  ...
  try {
    ...
    const pairs: Array<{
      mic: { data: Float32Array; sampleRate: number };
      speakers: { data: Float32Array; sampleRate: number };
      echoLevel: number;
    }> = [];
    ...
    for (let i = 0; i < 5; i++) {
      ...
      pairs.push({
        mic: { data: new Float32Array(micChunk), sampleRate: audio.sampleRate },
        speakers: {
          data: new Float32Array(speakerChunk),
          sampleRate: audio.sampleRate,
        },
    ...
    for (let i = 0; i < filtered.length; i++) {
      const originalSamples = pairs[i].mic.data.length;
      const filteredSamples = filtered[i].data?.length || 0;
      const _reduction =
...
async function main() {
  ...
  switch (args.mode) {
    case 'basic':
      await runBasicExample(args.audioFile, args.filterLength);
      break;

    case 'realtime':
      await runRealtimeExample(args.audioFile, args.filterLength);
      break;

    case 'streaming':
      await runStreamingExample(args.audioFile, args.filterLength);
      break;

    case 'batch':
      await runBatchExample(args.audioFile, args.filterLength);
      break;
