...
async function runBasicRAGExample() {
  ...
  const compiled = await knowledge.compileKnowledge({
    knowledgeId,
    records: programmingRecords as any,
  });
  ...
  for (const query of queries) {
    ...
    const results = await knowledge.search([knowledgeId], query, 2);
...
async function runMultiSourceKnowledgeExample() {
  ...
  for (const scenario of searchScenarios) {
    ...
    const results = await knowledge.search(scenario.sources, scenario.query, 3);
...
async function runBatchOperationsExample() {
  ...
  const results = await knowledge.compileBatch(knowledgeSets as any);
  ...
  for (const query of queries) {
    console.log(`\n  Query: "${query}"`);
    const searchResults = await knowledge.search(allIds, query, 2);
  ...
  console.log('\nVerifying removal...');
  try {
    await knowledge.getKnowledge({ ids: [allIds[0]] });
