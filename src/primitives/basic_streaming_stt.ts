...
async function runBasicExample(
  ...
  const textStream = await streamingSTT.recognizeSpeechStream(audioStream);
  ...
  for await (const transcription of textStream) {
    ...
    console.log(`[${transcriptionCount}] ${transcription}`);
  }
...
async function runRealtimeExample(
  ...
  for await (const transcription of textStream) {
    ...
    partialCount++;
    console.log(`[${elapsed}s] Partial ${partialCount}: "${transcription}"`);
  }
...
async function* createAudioStream(
  ...
  for (let i = 0; i < fullAudio.length; i += chunkSize) {
    ...
    yield {
      data: chunk as Float32Array<ArrayBuffer>,
      sampleRate,
    };
  }
}
