...
async function run() {
  const { mode, audioFile, language, apiKey } = parseArgs();

  try {
    switch (mode) {
      case 'basic':
        await runBasicExample(audioFile, language, apiKey);
        break;
      case 'streaming':
        await runStreamingExample(audioFile, language, apiKey);
        break;
      case 'language':
        await runLanguageExample(audioFile, apiKey);
        break;
      case 'batch':
        await runBatchExample(language, apiKey);
        break;
      default:
...
async function runBasicExample(
  ...
  // Create STT instance (flat config, no remoteConfig wrapper)
  ...
  const transcription = await stt.recognizeSpeechComplete({
    audioChunk: {
      data: float32ToBytes(audioData.channelData[0]),
 ...
 * @param {string} apiKey - API key
 */
async function runBatchExample(language: string, apiKey: string) {
...
function parseArgs(): {
  ...
  const language = argv.language || argv.languageCode || 'en-US';
  ...
  return { mode, audioFile, language, apiKey };
}
