...
Usage:
    npm run basic-llm -- \n
    --mode=completion|chat|streaming|tools|routing|reasoning[optional, default=completion] \n
  ...
  try {
    switch (mode) {
      ...
      case 'tools':
        ...
        break;
      case 'routing':
        await runRoutingExample(provider, model, apiKey);
        break;
      case 'reasoning':
        await runReasoningExample(provider, model, apiKey);
        break;
      default:
...
/**
 * Routing example - Demonstrates per-request routing options:
 * developerId, modelId override, userId, modelSelection, metadata
 *
 * @param {string} provider - LLM provider
 * @param {string} model - Model name
 * @param {string} apiKey - API key for the provider
...
async function runRoutingExample(
  ...
  console.log('\n=== Routing & Request Options Example ===\n');

  // LLM is created with a default model; per-request options can override it
  const llm = await LLM.create({
    remoteConfig: {
      modelId: `${provider}/${model}`,
      apiKey,
  ...
  // Example 1: Developer and user tracking
  console.log('Example 1: Developer and user tracking');
  console.log('─'.repeat(60));
  console.log(
    'Fields: developerId (provider tracking), userId (sticky routing)\n',
  );

  const result1 = await llm.generateContentChatComplete({
    messages: [{ role: 'user', content: 'What is 2 + 2?' }] as any,
    developerId: 'my-app-v1',
    userId: 'user-abc123',
  });
  console.log(result1);
  ...
  // Example 2: Per-request model override
  console.log('\n\nExample 2: Per-request model override');
  console.log('─'.repeat(60));
  console.log('Field: modelId — overrides the model set at LLM.create time\n');
  ...
  console.log(result2);
  console.log('User: Explain recursion in one sentence.');
  console.log(`Assistant: ${result2.content}`);
  console.log(`(Routed to: ${provider}/${model})`);

  // Example 3: Model selection with fallback and sort criteria
  console.log('\n\nExample 3: Model selection with fallback routing');
  console.log('─'.repeat(60));
  console.log(
    'Field: modelSelection — fallback models + sort by price then latency\n',
  );
  ...
  const result3 = await llm.generateContentChatComplete({
    ...
    modelSelection: {
      ...
      sort: [{ metric: SortMetric.Price }, { metric: SortMetric.Latency }],
    },
  });
  console.log(result3);

  console.log('User: Name three programming languages.');
  console.log(`Assistant: ${result3.content}`);

  // Example 4: Metadata for conditional routing
  console.log('\n\nExample 4: Metadata for conditional routing');
  console.log('─'.repeat(60));
  console.log(
    'Field: metadata — free-form JSON evaluated by CEL routing rules\n',
  );

  const result4 = await llm.generateContentChatComplete({
    messages: [
      { role: 'user', content: 'Give me a one-line Python hello world.' },
    ] as any,
    metadata: {
      region: 'us-east',
      tier: 'premium',
  ...
  console.log('User: Give me a one-line Python hello world.');
  console.log(`Assistant: ${result4.content}`);
  console.log(result4);
}

/**
 * Reasoning example - Chain-of-thought reasoning with thinking models
 *
 * @param {string} provider - LLM provider
 * @param {string} model - Model name (defaults to o3-mini for reasoning)
 * @param {string} apiKey - API key for the provider
 */
async function runReasoningExample(
  provider: string,
  model: string,
  apiKey: string,
) {
  const reasoningModel = model === 'gpt-4o-mini' ? 'o3-mini' : model;

  console.log('\n=== Reasoning Mode Example ===\n');
  console.log(`Using reasoning model: ${provider}/${reasoningModel}\n`);

  const llm = await LLM.create({
    remoteConfig: {
      modelId: `${provider}/${reasoningModel}`,
      apiKey,
      defaultTimeout: '120s',
      defaultConfig: {
        maxNewTokens: 2000,
  ...
  // Example 1: Reasoning with effort level HIGH
  console.log('Example 1: Reasoning with effort=HIGH');
  console.log('─'.repeat(60));
  ...
  const result1 = await llm.generateContentChatComplete({
    messages: [{ role: 'user', content: question1 }] as any,
    config: {
      maxNewTokens: 1000,
      reasoning: {
        effort: ReasoningConfig_Effort.HIGH,
      },
    },
  });

  if (result1.reasoning) {
    console.log('Reasoning (chain-of-thought):');
    console.log(result1.reasoning);
    console.log();
  }
  console.log(`Answer: ${result1.content}`);

  // Example 2: Reasoning with maxTokens control
  console.log('\n\nExample 2: Reasoning with maxTokens budget');
  console.log('─'.repeat(60));
  ...
  const result2 = await llm.generateContentChatComplete({
    messages: [{ role: 'user', content: question2 }] as any,
    config: {
      maxNewTokens: 1000,
      reasoning: {
        maxTokens: 500,
  ...
  console.log(`Answer: ${result2.content}`);

  // Example 3: Reasoning with exclude=true (internal reasoning only)
  console.log('\n\nExample 3: Internal-only reasoning (exclude=true)');
  console.log('─'.repeat(60));

  const question3 =
    'I have 12 coins. One is counterfeit and lighter. Using a balance scale, what is the minimum number of weighings to find it?';
  console.log(`User: ${question3}\n`);

  const result3 = await llm.generateContentChatComplete({
    messages: [{ role: 'user', content: question3 }] as any,
    config: {
      maxNewTokens: 1000,
      reasoning: {
        effort: ReasoningConfig_Effort.HIGH,
  ...
  if (result3.reasoning) {
    console.log('Reasoning returned (unexpected with exclude=true):');
    console.log(result3.reasoning);
  } else {
    console.log(
      '(reasoning excluded from response — model still reasons internally)',
    );
  }
  console.log(`\nAnswer: ${result3.content}`);

  // Example 4: Comparing effort levels
  console.log('\n\nExample 4: Effort level comparison');
  console.log('─'.repeat(60));

  const question4 = 'What is 7 * 8?';
  console.log(`Question: ${question4}\n`);

  const efforts: [string, ReasoningConfig_Effort][] = [
    ['MINIMAL', ReasoningConfig_Effort.MINIMAL],
    ['MEDIUM', ReasoningConfig_Effort.MEDIUM],
    ['HIGH', ReasoningConfig_Effort.HIGH],
  ];

  for (const [label, effort] of efforts) {
    const start = Date.now();
    const result = await llm.generateContentChatComplete({
      messages: [{ role: 'user', content: question4 }] as any,
      config: {
        maxNewTokens: 500,
        reasoning: { effort },
      },
    ...
    console.log(`  effort=${label} (${elapsed}ms):`);
    console.log(`    Answer: ${result.content?.trim()}`);
    if (result.reasoning) {
      const lines = result.reasoning.trim().split('\n');
      console.log(
        `    Reasoning: ${lines[0]}${lines.length > 1 ? ` ... (${lines.length} lines)` : ''}`,
      );
    }
    console.log();
  }
