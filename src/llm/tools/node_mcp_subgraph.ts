...
const usage = `
Usage:
    npm run node-mcp-subgraph "What's the weather like in San Francisco?" -- --modelName=gpt-4o-mini --provider=openai
...
Other examples:
   npm run node-mcp-subgraph "please structure this content: teacher, Elizabeth, hobby, basketball, favorite food is hamburger" -- --modelName=gpt-4o-mini --provider=openai
   npm run node-mcp-subgraph "Hello, how are you?" -- --modelName=gpt-4o-mini --provider=openai
`;
...
async function run() {
  ...
  for await (const result of outputStream) {
    await result.processResponse({
      Custom: (messages: GraphTypes.Custom<Message[]>) => {
        console.log('\n✅ Agent response:');
