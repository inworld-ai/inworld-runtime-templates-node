...
import { exitWithError, findNpxPath } from '../../shared/helpers/cli_helpers';
...
const usage = `
Usage:
    npm run node-mcp-call-tool -- [--server=everything|brave-stdio|brave] [--port=<port>] [--help]
...
Examples:
    npm run node-mcp-call-tool -- --server=everything --port=3001
    npm run node-mcp-call-tool -- --server=brave --port=8080
    npm run node-mcp-call-tool -- --server=brave-stdio`;
...
async function run() {
  ...
  const { outputStream } = await graph.start(
    new GraphTypes.ToolCallRequest({ toolCalls: toolCalls }),
  );
