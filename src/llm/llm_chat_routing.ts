...
Usage:
    ...
    --metadata=<json>[optional, JSON object for conditional routing] \n
    --requestId=<id>[optional, request ID for router-side tracing]
...
Examples:
    ...
    # Route with metadata and explicit request ID
    npm run node-llm-chat-routing "Tell me a joke" -- --metadata='{"userTier":"pro","locale":"en-US"}' --requestId="req-123"
    `;
      ...
      LLMChatResponse: (response: GraphTypes.LLMChatResponse) => {
        ...
        if (response.metadata) {
          ...
          if (
            ...
            response.metadata.attempts.forEach(
              (attempt: Record<string, unknown>, idx: number) => {
                ...
                const status = typedAttempt.success ? '✅' : '❌';
                const modelId = typedAttempt.modelId
                  ? `${typedAttempt.modelId.provider}/${typedAttempt.modelId.modelName}`
                  : 'unknown';
                console.log(`    ${idx + 1}. ${status} ${modelId}`);
                if (typedAttempt.timeToFirstTokenMs !== undefined) {
                  console.log(
                    `       Time to First Token: ${typedAttempt.timeToFirstTokenMs}ms`,
                  );
                }
                if (typedAttempt.errorMessage) {
                  console.log(`       Error: ${typedAttempt.errorMessage}`);
                }
                if (typedAttempt.warnings && typedAttempt.warnings.length > 0) {
                  console.log(
                    `       Warnings: ${typedAttempt.warnings.join(', ')}`,
                  );
                }
              },
            );
...
function parseArgs(): {
  ...
  const metadata = parseMetadataArg(argv.metadata);
  const requestId =
    argv.requestId !== undefined ? String(argv.requestId) : undefined;
  ...
  for (const sort of sortArgs) {
    ...
    switch (metric) {
      case 'INTELLIGENCE':
        metricValue = SortMetric.Intelligence;
        break;
      case 'PRICE':
        metricValue = SortMetric.Price;
        break;
      case 'LATENCY':
        metricValue = SortMetric.Latency;
        break;
      case 'THROUGHPUT':
        metricValue = SortMetric.Throughput;
        break;
      case 'MATH':
        metricValue = SortMetric.Math;
        break;
      case 'CODING':
        metricValue = SortMetric.Coding;
        break;
      default:
        console.warn(
  ...
  if (autoMode) {
    for (const model of modelArgs) {
      ...
      if (modelProvider && modelModelName) {
        fallbackModels.push(`${modelProvider}/${modelModelName}`);
      } else {
        console.warn(
    ...
    for (const fallback of fallbackArgs) {
      ...
      if (fbProvider && fbModelName) {
        fallbackModels.push(`${fbProvider}/${fbModelName}`);
      } else {
        console.warn(
    ...
    if (!fallbackModels.length) {
      fallbackModels.push('openai/gpt-4.1-mini');
    }
  }
...
function parseMetadataArg(
  ...
  if (typeof rawMetadata !== 'string') {
    throw new Error(
      `Invalid --metadata value. Expected a JSON object string.\n${usage}`,
    );
  }
