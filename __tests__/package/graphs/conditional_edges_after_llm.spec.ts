...
const script = 'conditional-edges-after-llm';

function checkConditionalEdgesOutputValidation(output: string) {
...
function extractLLMResult(output: string): {
  ...
  // Handle both single and double quotes around content value
...
function extractCustomNodeResult(output: string): {
  ...
  // Handle both single and double quotes around data value
...
function validateConditionalLogic(llmContent: string, customMessage: string) {
...
describe('Tests for conditional-edges-after-llm template', () => {
  it('should execute conditional edges workflow successfully', async () => {
    await allure.suite('Tests for conditional-edges-after-llm template');

    const { output, errorOutput, exitCode } = await runExample(script, []);

    checkCommandOutputDetailed(errorOutput, exitCode, output);
    checkConditionalEdgesOutputValidation(output);
    ...
    validateConditionalLogic(llmResult.content, customResult.message);
  ...
  it('should show help when --help flag is provided', async () => {
    ...
    expect(output).toContain('npm run conditional-edges-after-llm');
    expect(output).toContain('Description:');
    expect(output).toContain(
      'demonstrates how to create a graph with conditional edges',
    );
  ...
  it('should handle multiple test runs with different numbers', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const path = number > 50 ? 'greater' : 'lessEqual';
  ...
  it('should validate conditional edge expressions work correctly', async () => {
    await allure.suite('Tests for conditional-edges-after-llm template');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // Verify the condition expression logic
      let conditionMet = false;
      if (number > 50) {
        conditionMet = message.includes('Generated number is greater than 50');
      } else {
        conditionMet = message.includes(
