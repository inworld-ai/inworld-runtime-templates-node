...
// Import custom nodes
import { LLMExecutorCustomNode } from './llm_executor_node';
import { LLMRouterCustomNode } from './llm_router_node';
...
async function run() {
  ...
  const llmRoutingSubgraph = new SubgraphBuilder('llm_routing_subgraph')
    ...
    .addEdge(executorNode, routerNode, {
      conditionExpression:
        '!input.value.success && input.value.currentStrategyIndex < ' +
        (strategies.length - 1),
      optional: true,
      loop: true,
    })
